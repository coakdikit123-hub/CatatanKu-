...
let exportToGoogleSheets = null;
try {
  const sheetsModule = require('./google-sheets');
  exportToGoogleSheets = sheetsModule.exportToGoogleSheets;
  console.log('✅ Google Sheets module loaded');
} catch (e) {
  console.warn('⚠️ google-sheets.js tidak tersedia:', e.message);
...
const DEFAULT_CATEGORIES = [
  'dining', 'shopping', 'transport', 'bills', 'fun', 'health', 'gift', 'other'
];
...
function parseTransaction(text, userId) {
  ...
  // Ekstrak kategori dari note (kata pertama)
...
if (BOT_TOKEN && Telegraf) {
  try {
    ...
    // ============================================================
    // HANDLER: pesan teks (transaksi & edit OCR)
    // ============================================================
...
if (exportToGoogleSheets) {
  ...
  app.get('/api/export-sheets/:userId', (req, res) => {
